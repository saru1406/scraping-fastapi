...
class LancersRepository:
    ...
    async def fetch_lancers(self):
        url = (
            self.base_url
            + "/work/search?open=1&ref=header_menu&work_rank%5B0%5D=2&work_rank%5B1%5D=3&work_rank%5B2%5D=0&show_description=1&page="
        )
        page = 1
        all_link = []
        all_title = []
        ...
        async with aiohttp.ClientSession() as session:
            while True:
                async with session.get(url + str(page)) as response:
                    ...
                    jobs = soup.find_all(
                        "div", class_="p-search-job-media c-media c-media--item"
                    )
                    for job in jobs:

                        # タイトル、リンク取得
                        job_link = job.find(
                            "a", class_="p-search-job-media__title c-media__title"
                        )
                        if job_link:
                        ...
                        # 応募数取得
                        job_limits = job.find(
                            "div", class_="p-search-job-media__job-status"
                        )
                        if job_limits:
                            job_limit = job_limits.find(
                                "div", class_="c-media__job-stats-item"
                            )
                            if job_limit:
